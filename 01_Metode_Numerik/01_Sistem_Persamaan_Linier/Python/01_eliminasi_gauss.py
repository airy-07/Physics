...
def eliminasi_gauss(A, b, verbose=False):
    ...
    # Fungsi pembantu untuk mencetak matriks augmented secara rapi
    def cetak_augmented(mat_A, vec_b):
        for idx in range(n):
            A_row = [0.0 if np.abs(val) < 1e-12 else val for val in mat_A[idx]]
            b_val = 0.0 if np.abs(vec_b[idx]) < 1e-12 else vec_b[idx]
            coef_str = " ".join(f"{val:9.4f}" for val in A_row)
            print(f"    [ {coef_str}  │  {b_val:9.4f} ]")

    if verbose:
        print("\n" + "=" * 55)
        print(" LANGKAH-LANGKAH NUMERIK: ELIMINASI MAJU")
        print("=" * 55)
    ...
    for k in range(n):
        ...
        if pivot_row != k:
            if verbose:
                print(f"  * 🔄 Tukar Baris {k+1} <-> Baris {pivot_row+1} (karena |{A_mod[pivot_row, k]:.4f}| terbesar pada kolom {k+1})")
            ...
            if verbose:
                print(f"  * ✔️ Baris {k+1} sudah memuat pivot terbesar ({A_mod[k, k]:.4f}). Tidak ada baris ditukar.")
        ...
        for i in range(k + 1, n):
            ...
            if np.abs(factor) > 1e-15:
                eliminated_any = True
                if verbose:
                    print(f"  * ⚙️ OBE: Baris {i+1} <- Baris {i+1} - ({factor:.4f}) * Baris {k+1}")
        ...
        if verbose:
            if eliminated_any:
                print("  * Hasil matriks setelah eliminasi kolom:")
                cetak_augmented(A_mod, b_mod)
            else:
                print("  * Elemen di bawah diagonal kolom ini sudah nol.")
            
    # --- Tahap 2: Substitusi Mundur ---
    if verbose:
        print("\n" + "=" * 55)
        print(" LANGKAH-LANGKAH NUMERIK: SUBSTITUSI MUNDUR")
        print("=" * 55)
    ...
    for i in range(n - 2, -1, -1):
        ...
        if verbose:
            print(f"  * x[{i+1}] = ({b_mod[i]:.4f} - ({sum_terms:.4f})) / {A_mod[i, i]:.4f} = {x[i]:.6f}")
...
if __name__ == '__main__':
    ...
    try:
        ...
        # Opsi tampilkan langkah
        show_steps = input("\nTampilkan langkah-langkah perhitungan? (y/n): ").strip().lower() == 'y'
        
        # Selesaikan SPL
        x = eliminasi_gauss(A, b, verbose=show_steps)
